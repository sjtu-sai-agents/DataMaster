...
echo "[start] no-red task=${AGENT_TYPE}, limit=${TIME_LIMIT_SECS}s"
...
AGENT_TYPE="$1"
CONFIG="configs/ml_master_datatree/yaml_configs/${AGENT_TYPE}/config_${AGENT_TYPE}_no_red.yaml"
...
if [ "$status" -eq 124 ]; then
    echo "[timeout] no-red task ${AGENT_TYPE} timed out after 12h"
    send_feishu "EvoMaster no-red task ${AGENT_TYPE} timed out after 12h"
    exit 124
elif [ "$status" -eq 137 ]; then
    echo "[killed] no-red task ${AGENT_TYPE} was killed with exit code 137"
    send_feishu "EvoMaster no-red task ${AGENT_TYPE} was killed with exit code 137, likely OOM/system kill"
    exit 137
elif [ "$status" -eq 0 ]; then
    echo "[done] no-red task ${AGENT_TYPE} finished successfully"
    send_feishu "EvoMaster no-red task ${AGENT_TYPE} finished successfully"
    exit 0
else
    echo "[failed] no-red task ${AGENT_TYPE} failed with exit code ${status}"
    send_feishu "EvoMaster no-red task ${AGENT_TYPE} failed with exit code ${status}"
    exit "$status"
fi