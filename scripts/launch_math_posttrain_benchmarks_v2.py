...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Launch math_posttrain_datatree_v2 runs. Use single-run mode with --benchmark/--gpu, "
            "or batch mode with the default benchmark->GPU map."
        )
    )
    parser.add_argument(
        "--base-config",
        default="configs/math_posttrain_datatree_v2/config_gpu2.yaml",
    ...
    *,
    ...
    def abs_repo(path: str | None) -> str | None:
        ...
        if raw.startswith("prompts/"):
            return str(repo_root / "playground" / "math_posttrain_datatree_v2" / raw)
        if raw.startswith("mcp_config"):
            return str(repo_root / "configs" / "math_posttrain_datatree_v2" / raw)
        if raw.startswith("./"):
...
def build_launch_env(repo_root: Path, args: argparse.Namespace) -> dict[str, str]:
    ...
    env.setdefault("MATH_PT_SHARED_CACHE", hf_home)
    return env
...
def main() -> int:
    ...
    for benchmark, device in benchmark_device_pairs:
        run_dir = repo_root / "runs" / f"math_posttrain_datatree_v2_{benchmark}_gpu{device}{suffix_part}_{timestamp}"
