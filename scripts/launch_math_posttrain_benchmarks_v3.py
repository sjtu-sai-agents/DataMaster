...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Launch math_posttrain_datatree_v3 runs. Use single-run mode with --benchmark/--gpu, "
            "or batch mode with the default benchmark->GPU map."
        )
    )
    parser.add_argument(
        "--base-config",
        default="configs/math_posttrain_datatree_v3/config_gpu2.yaml",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override evaluation.posttrainbench.limit (number of eval examples).",
    )
    parser.add_argument(
        "--cutoff-len",
        type=int,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Hugging Face cache root for the launched run. Defaults to "
            "<repo_parent>/.cache/huggingface unless HF_HOME is already set."
        ),
    )
    parser.add_argument(
        "--shared-cache",
        ...
        default=None,
        help=(
            "Shared materialized dataset cache root. Defaults to "
            "<repo_parent>/.cache/math_posttrain_datatree_v3 unless MATH_PT_SHARED_CACHE is already set."
        ),
    )
    parser.add_argument(
        "--dry-run",
    ...
    *,
    ...
    def abs_repo(path: str | None) -> str | None:
        ...
        if raw.startswith("prompts/"):
            return str(repo_root / "playground" / "math_posttrain_datatree_v3" / raw)
        if raw.startswith("mcp_config"):
            return str(repo_root / "configs" / "math_posttrain_datatree_v3" / raw)
        if raw.startswith("./"):
    ...
    for agent_name in ("red", "black", "observer"):
        ...
        if isinstance(tools, dict) and tools.get("mcp"):
    ...
    if eval_limit is not None:
        posttrainbench["limit"] = eval_limit
...
def build_launch_env(repo_root: Path, args: argparse.Namespace) -> dict[str, str]:
    env = os.environ.copy()
    default_hf_home = repo_root.parent / ".cache" / "huggingface"
    hf_home = args.hf_home or env.get("HF_HOME") or str(default_hf_home)
    env["HF_HOME"] = hf_home
    env.setdefault("HF_ENDPOINT", "https://hf-mirror.com")
    env.setdefault("HF_HUB_URL", env["HF_ENDPOINT"])
    ...
    default_shared_cache = repo_root.parent / ".cache" / "math_posttrain_datatree_v3"
    env["MATH_PT_SHARED_CACHE"] = args.shared_cache or env.get("MATH_PT_SHARED_CACHE") or str(default_shared_cache)
    return env
...
def main() -> int:
    ...
    launch_env = build_launch_env(repo_root, args)
    ...
    for benchmark, device in benchmark_device_pairs:
        run_dir = repo_root / "runs" / f"math_posttrain_datatree_v3_{benchmark}_gpu{device}{suffix_part}_{timestamp}"
        ...
        print(
            "cache "
            f"HF_HOME={launch_env.get('HF_HOME')} "
            f"HF_DATASETS_CACHE={launch_env.get('HF_DATASETS_CACHE')} "
            f"MATH_PT_SHARED_CACHE={launch_env.get('MATH_PT_SHARED_CACHE')}"
        )
        print(" ".join(cmd))
        if not args.dry_run:
            proc = subprocess.Popen(cmd, cwd=repo_root, env=launch_env)
