...
def run_single_task(agent_name: str, config_path: Path, run_dir: Path,
                    ...
                    initial_code: str | None = None, initial_instruction: str | None = None,
                    test_feedback: bool = False, force_minimize: bool = False,
    ...
    Args:
        ...
        images: 图片文件路径列表（可选）
        initial_code: Initial 节点的初始代码文件路径（可选）
        initial_instruction: Initial 节点的自然语言指令（可选）
        test_feedback: 启用测试集反馈模式（可选）
        force_minimize: 强制 metric 越小越好（可选）
        force_maximize: 强制 metric 越大越好（可选）

    Returns:
        任务结果字典
    """
    # 参数校验：test-feedback 启用时必须指定方向
    if test_feedback and not (force_minimize or force_maximize):
        print("❌ 错误：--test-feedback 启用时必须指定 --force-minimize 或 --force-maximize")
        sys.exit(1)

    # 确定 force_direction
    force_direction = None
    if force_minimize:
        force_direction = "minimize"
    elif force_maximize:
        force_direction = "maximize"
    ...
    try:
        ...
        # 设置 initial 相关配置（如果 playground 支持）
        if hasattr(playground, 'set_initial_config'):
            playground.set_initial_config(initial_code, initial_instruction)

        # 设置 test_feedback 相关配置（如果 playground 支持）
        if hasattr(playground, 'set_test_feedback_config'):
            playground.set_test_feedback_config(test_feedback, force_direction)
...
def main():
    ...
    # 3.5. 验证和处理 initial_code 和 initial_instruction（如果提供）
    initial_code = None
    initial_instruction = None

    if args.initial_code:
        initial_code_path = Path(args.initial_code)
        if not initial_code_path.exists():
            logger.error(f"初始代码文件不存在: {initial_code_path}")
            sys.exit(1)
        try:
            with open(initial_code_path, 'r', encoding='utf-8') as f:
                initial_code = f.read()
            logger.info(f"✅ 加载了初始代码文件: {initial_code_path}")
        except Exception as e:
            logger.error(f"读取初始代码文件失败: {e}")
            sys.exit(1)

    if args.initial_instruction:
        initial_instruction = args.initial_instruction
        logger.info(f"✅ 使用初始指令: {initial_instruction[:100]}...")

    if not initial_code and not initial_instruction:
        logger.info("📝 未提供初始代码或指令，将使用自由模式")
    ...
    try:
        if len(tasks) > 1 and args.parallel:
            ...
            results = run_tasks_parallel(args.agent, config_path, run_dir, tasks, images=images,
                                        initial_code=initial_code, initial_instruction=initial_instruction,
                                        test_feedback=args.test_feedback, force_minimize=args.force_minimize,
                                        force_maximize=args.force_maximize)
        else:
            # 串行模式（包括单任务）
            if len(tasks) > 1:
                logger.info("🔄 串行执行任务...")
