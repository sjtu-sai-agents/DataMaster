...
from .exp.improve_with_data_exp import ImproveWithDataExp
from .exp.data_search_exp import DataSearchExp

logger = logging.getLogger(__name__)


@register_playground("ml_master_warmup")
class MLMasterWarmupPlayground(BasePlayground):
    ...
    def __init__(self, config_dir: Path | None = None, config_path: Path | None = None):
        if config_path is None and config_dir is None:
            config_dir = Path(__file__).parent.parent.parent.parent / "configs" / "ml_master_warmup"
        super().__init__(config_dir=config_dir, config_path=config_path)
        self.logger = logging.getLogger(self.__class__.__name__)
        self.agents.declare("draft_agent", "debug_agent", "improve_agent", "metric_agent", "data_agent")
...
# --------------------------- 初始化 --------------------------- #
    def setup(self) -> None:
        self.logger.info("Setting up MLMasterWarmupPlayground...")
        self._setup_session()
        self._setup_agents()
        ...
        if self.agents.get("data_agent") is None:
            self.logger.info("config.agents 未配置 data_agent，data_search 将回退复用 improve_agent")
    ...
    def _create_worker_agents(self, worker_index: int) -> dict[str, Agent]:
        data_template = self.agents.get("data_agent") or self.agents.get("improve_agent")
        if data_template is None:
            raise ValueError("config.agents 缺少 improve（以及可选 data）配置，无法创建 data worker agent")
        return {
            "draft": self.copy_agent(self.agents.get("draft_agent"), new_agent_name=f"draft_worker_{worker_index}"),
            "debug": self.copy_agent(self.agents.get("debug_agent"), new_agent_name=f"debug_worker_{worker_index}"),
            "improve": self.copy_agent(self.agents.get("improve_agent"), new_agent_name=f"improve_worker_{worker_index}"),
            "metric": self.copy_agent(self.agents.get("metric_agent"), new_agent_name=f"metric_worker_{worker_index}"),
            "data": self.copy_agent(data_template, new_agent_name=f"data_worker_{worker_index}"),
        }
    ...
    @staticmethod
    def _has_child_stage(target: Any, stage: str) -> bool:
        return any(getattr(child, "stage", None) == stage for child in (getattr(target, "children", None) or []))

    @classmethod
    def _select_stage_and_inputs(cls, target: Any) -> tuple[str, str, str, str]:
        if target.stage == "root":
            return "draft", "", "", ""
        issue = getattr(target, "grading_issue", "") or ""
        if target.stage == "data_search":
            if getattr(target, "data_ok", False):
                return "improve_with_data", getattr(target, "code", ""), getattr(target, "stdout", ""), issue
        ...
        if not cls._has_child_stage(target, "data_search"):
            return "data_search", getattr(target, "code", ""), getattr(target, "stdout", ""), issue
        if getattr(target, "data_ok", False) and not cls._has_child_stage(target, "improve_with_data"):
            return "improve_with_data", getattr(target, "code", ""), getattr(target, "stdout", ""), issue
        return "improve", getattr(target, "code", ""), getattr(target, "stdout", ""), issue
    ...
    def _run_one_node(
        ...
        *,
        ...
        if stage == "improve_with_data":
            exp = ImproveWithDataExp(
                ...
                while True:
                    ...
                    with state_lock:
                        ...
                        if should_wait:
                            ...
                            data_context = getattr(target, "data_context", None)
                    if should_wait:
                        time.sleep(0.1)
                        continue
                    ...
                    try:
                        ...
                        with state_lock:
                            ...
                            node.data_context = res.get("data_context")
                            node.data_ok = res.get("data_ok")
                            node.stdout = res.get("exec", {}).get("stdout", "")
                            node.exit_code = res.get("exec", {}).get("exit_code", None)

                            copied = None
                            if stage != "data_search":
