...
StageLiteral = Literal["root", "draft", "debug", "improve", "data_search", "improve_with_data"]
...
class UCTSearchManager:
    ...
    def create_child(
        ...
        """
        parent.expect_child()
        node = UCTNode(stage=stage, action_type=stage, plan=plan, code=code, parent=parent)
    ...
    def ingest_result(
        ...
        *,
        ...
        is_data_search = node.stage == "data_search"
        if is_data_search:
            node.metric = WorstMetricValue()
            node.is_buggy = False
            node.is_valid = bool(node.data_ok)
            node.continue_improve = bool(node.data_ok)
            node.is_terminal = not bool(node.data_ok)
        else:
        ...
        if node.parent and node.parent.is_buggy and node.is_buggy is False:
            ...
            # Keep parent's own buggy state unchanged; child success is tracked separately.
            # Otherwise parent may appear "ok" with empty metric/submission context.
        ...
        if not is_data_search and not node.is_buggy and not self._check_metric_valid(node):
            node.metric = WorstMetricValue()
            node.is_buggy = True
        ...
        reward = (0.2 if node.data_ok else -0.2) if is_data_search else self._get_node_reward(node)
        ...
        if self.snapshot_fn:
            def _submission_path_for(n: UCTNode) -> Optional[Path]:
                if not self.submission_dir:
                    return None
                p = self.submission_dir / f"submission_{n.id}.csv"
                return p if p.exists() else None

            current = node
            while current:
                sub = _submission_path_for(current)
                current_review = MetricReview(
                    metric=getattr(getattr(current, "metric", None), "value", None),
                    maximize=getattr(getattr(current, "metric", None), "maximize", True),
                    is_bug=bool(current.is_buggy),
                    has_submission=sub is not None,
                    summary=current.analysis or "",
                )
                try:
                    self.snapshot_fn(current, sub, current_review, reward)
