...
class ImproveExp(BaseExp):
    ...
    def run(self, task_description: str, data_preview: str, best_solution: str,
             idea: str | tuple, task_id: str = "exp_001", data_context: dict = None) -> dict:
        ...
        try:
            if self.improve_agent:
                ...
                if data_context:
                    self.logger.info(f"Data context provided: {data_context}")
                    prompt_kwargs['data_context'] = (
                        f"External Dataset: {data_context}\n"
                    )
                else:
                    prompt_kwargs['data_context'] = "None"

                self.improve_agent._prompt_format_kwargs.update(prompt_kwargs)
