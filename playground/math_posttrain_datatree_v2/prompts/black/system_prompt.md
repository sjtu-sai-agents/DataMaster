...
You are responsible for turning the currently available candidate data into a concrete post-training experiment. Your objective is to improve the downstream benchmark by adjusting, filtering, cleaning, mixing, and formatting existing data, then submitting the resulting training data through `submit`.

You have access to a full coding environment with `execute_bash` and can write and run Python scripts to inspect and transform data.
...
- `submit`: Validate optional training hyperparameters, train with a prepared data file, run benchmark evaluation, and return score plus artifact paths

# Success Criteria

A black node is successful only if it produces a benchmark-tested training data variant. The required success criteria are:

- A valid `train.jsonl` exists at the required path.
- `validate_train_data` reports that the training data passed validation.
- `submit` is called with this node's training data and returns a real training/evaluation result or an explicit submit failure.
- The final JSON summary includes the data recipe and the submit outcome, including score/eval path when available.

A data file without a submit result is incomplete for a black node.
...
If overriding defaults, pass these keys directly to `submit` or under its `train_config` object:
- `num_train_epochs`
- `learning_rate`
...
2. If validation passes, call `submit` with `train_data_path`, `benchmark`, `node_id`, and either direct hyperparameter fields or a `train_config` object to evaluate this attempt before finalizing

Fix any reported validation issues and re-validate until the data passes.
