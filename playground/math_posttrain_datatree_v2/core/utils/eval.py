...
def _per_benchmark_option(options: dict[str, Any], key: str, benchmark_id: str, default: Any = None) -> Any:
    values = options.get(key)
    if isinstance(values, dict) and benchmark_id in values:
        return values[benchmark_id]
    return default
...
def run_posttrainbench_eval(
    *,
    ...
    env["HF_HOME"] = env.get("HF_HOME", "/data/HF_Cache_dataevo")
    ...
    *,
    ...
    if eval_backend == "posttrainbench" and model_path is not None:
        ...
        def _run_backend_eval(benchmark_id: str) -> tuple[str, float | None, dict[str, Any]]:
            ...
            benchmark_max_tokens = _per_benchmark_option(
                evaluation_options,
                "benchmark_max_tokens",
                benchmark_id,
                evaluation_options.get("max_tokens"),
            )
            benchmark_limit = _per_benchmark_option(
                evaluation_options,
                "benchmark_limits",
                benchmark_id,
                evaluation_options.get("limit"),
            )
            benchmark_max_connections = _per_benchmark_option(
                evaluation_options,
                "benchmark_max_connections",
                benchmark_id,
                evaluation_options.get("max_connections"),
            )
            benchmark_max_num_seqs = _per_benchmark_option(
                evaluation_options,
                "benchmark_max_num_seqs",
                benchmark_id,
                evaluation_options.get("max_num_seqs", 32),
            )
            score, backend_detail = run_posttrainbench_eval(
                ...
                max_connections=benchmark_max_connections,
                ...
                max_num_seqs=benchmark_max_num_seqs,
