...
def normalize_data_access_config(config: dict[str, Any] | None) -> dict[str, Any]:
    ...
    hf_cache = str(raw.get("hf_cache") or raw.get("hf_datasets_cache") or os.getenv("HF_DATASETS_CACHE") or "").strip()
    return {
...
def _load_via_datasets_library(
    ...
    *,
    ...
    os.environ["HF_HUB_DOWNLOAD_TIMEOUT"] = "60"  # 1 minute (reduced from 5 minutes)
...
def materialize_dataset_entry(
    ...
    *,
    ...
    shared_cache_root = os.getenv("MATH_PT_SHARED_CACHE") or os.getenv("HF_HOME")
    if shared_cache_root:
        cache_dir = Path(shared_cache_root) / "materialized_datasets"
        LOGGER.info("Using shared materialized dataset cache: %s", cache_dir)
    else:
    ...
    hf_cache = str(data_access.get("hf_cache") or "").strip()
    ...
    *,
    ...
    def _record_metadata_type(field: str, value: Any, lineno: int) -> None:
        ...
        if field == "tags":
            if not isinstance(value, list) or any(not isinstance(item, str) for item in value):
                warnings.append(f"line {lineno} has non-standard metadata.tags: expected list[str]")
                return
            current_type = "list[str]"
        elif field in {"source_id", "topic", "difficulty", "raw_id"}:
            if not isinstance(value, str):
                warnings.append(f"line {lineno} has non-string metadata.{field}; it will be treated as diagnostic metadata")
                return
            current_type = "str"
        else:
        ...
        for lineno, raw_line in enumerate(train_path.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            for field in ("instruction", "input", "output"):
                ...
                if metadata is None:
                    metadata = {}
                if not isinstance(metadata, dict):
                    warnings.append(f"line {lineno} has non-object metadata; source will be treated as unknown")
                else:
                    source_id = metadata.get("source_id")
                    normalized_source_id = str(source_id or "unknown").strip() or "unknown"
    ...
    if prep_report_path:
        ...
        if not prep_path.exists():
            warnings.append(f"prep report not found: {prep_path}")
        else:
            try:
                prep_report = json.loads(prep_path.read_text(encoding="utf-8"))
            except json.JSONDecodeError as exc:
                warnings.append(f"prep report is not valid JSON: {exc}")
            if isinstance(prep_report, dict):
                for key in (
                    ...
                    if key not in prep_report:
                        warnings.append(f"prep report missing key: {key}")
                ...
                if raw_rows_seen_issue:
                    warnings.append(raw_rows_seen_issue)
                elif raw_rows_seen_total is None:
                    warnings.append("prep report raw_rows_seen must be present")
                ...
                if isinstance(rows_written, int):
                    ...
                    warnings.append("prep report rows_written must be an integer")
            else:
                warnings.append("prep report must be a JSON object")
...
def prepare_dataset_probe(
    ...
    *,
    ...
    # Step 1: Identify entries that need materialization
    entries_to_materialize = []
    for entry in dataset_entries:
        full_local_path = _get_entry_full_local_path(entry)
        ...
        if probe_sample_rows_path and _materialized_path_has_rows(Path(probe_sample_rows_path)):
            continue
        entries_to_materialize.append(entry)

    # Step 2: Parallel materialization
    materialized_paths = {}
    if entries_to_materialize:
        LOGGER.info("Materializing %d datasets in parallel (max_workers=5)...", len(entries_to_materialize))

        def _materialize_one(entry):
            source_id = str(entry.get("source_id") or entry.get("name") or "unknown_source")
            try:
                LOGGER.info("Starting materialization: %s", source_id)
                ...
                LOGGER.info("Completed materialization: %s", source_id)
                return source_id, probe_source_path, None
            except Exception as exc:
                LOGGER.warning("Failed to materialize %s: %s", source_id, exc)
                return source_id, "", exc

        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = {executor.submit(_materialize_one, entry): entry for entry in entries_to_materialize}
            for future in as_completed(futures):
                source_id, probe_source_path, error = future.result()
                if not error:
                    materialized_paths[source_id] = probe_source_path

    # Step 3: Process all entries (now with materialized data)
    ...
    for entry in dataset_entries:
        ...
        if not probe_source_path and not probe_sample_rows_path:
            probe_source_path = materialized_paths.get(source_id, "")
