...
from ..utils.types import DatasetEntry
from . import NodeExp
...
def _count_rows(path_value: str) -> int | None:
    if not path_value:
        return None
...
def _compact_sample(row: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in row.items():
        ...
        if isinstance(value, str):
            sample[str(key)] = value[:500].rstrip()
        elif isinstance(value, (int, float, bool)) or value is None:
            sample[str(key)] = value
        elif isinstance(value, list):
            sample[str(key)] = value[:3]
        elif isinstance(value, dict):
            sample[str(key)] = {str(k): v for k, v in list(value.items())[:4]}
    return sample


def _merge_probe_into_global_manifest(
    global_pool_manifest: dict[str, Any],
    ...
    datasets = global_pool_manifest.get("datasets")
    if not isinstance(datasets, list):
        return global_pool_manifest
    for entry in datasets:
        if not isinstance(entry, dict):
            continue
        source_id = str(entry.get("source_id") or entry.get("name") or "")
        probe_info = probe_by_source.get(source_id)
        if not probe_info:
            continue
        local_path = str(probe_info.get("full_local_path") or probe_info.get("local_path") or "")
        if local_path:
            entry["local_path"] = local_path
            entry["full_local_path"] = local_path
        preview_rows = probe_info.get("preview_rows") if isinstance(probe_info.get("preview_rows"), list) else []
        schema_keys = [str(item) for item in (probe_info.get("schema_keys") or [])]
        if preview_rows and isinstance(preview_rows[0], dict):
            entry["sample"] = {
                "keys": schema_keys,
                "row": _compact_sample(preview_rows[0]),
            }
        quality = entry.get("quality_signals") if isinstance(entry.get("quality_signals"), dict) else {}
        hint_parts = [
            str(quality.get("distribution_match") or "").strip(),
            str(quality.get("sample_evidence") or "").strip(),
            str(quality.get("relevance") or "").strip(),
        ]
        rows = _count_rows(local_path)
        notes = entry.get("notes") if isinstance(entry.get("notes"), dict) else {}
        if rows is not None:
            notes["rows"] = rows
        if entry.get("coverage_tags"):
            notes["good_for"] = [str(item) for item in (entry.get("coverage_tags") or [])]
        hint = " ".join(part for part in hint_parts if part)
        if hint:
            notes["use"] = hint[:500]
        entry["notes"] = notes
        entry.pop("probe_sample_rows_path", None)
    return global_pool_manifest
...
class RedExp(NodeExp):
    ...
    def run(self, task_description: str) -> dict:
        ...
        global_pool_manifest = load_json_payload(self.global_pool_manifest_path)
        raw_global_entries = global_pool_manifest.get("datasets") if isinstance(global_pool_manifest.get("datasets"), list) else []
        entries = [
            entry
            for item in raw_global_entries
            if isinstance(item, dict) and (entry := self._coerce_dataset_entry(item)) is not None
        ]
        if not entries:
        ...
        global_pool_manifest = _merge_probe_into_global_manifest(global_pool_manifest, pool_probe_payload)
        prepared_by_source = {
        ...
        return {
            ...
            "exec": {
                "stdout": (
                    f"global_pool={self.global_pool_manifest_path} "
                    f"total_sources={pool_summary.get('source_count', 0)} "
                    f"prepared_rows={int(pool_probe_payload.get('total_sample_rows') or 0)}"
                ),
            ...
            "metric_detail": {
                ...
                "manifest_path": str(self.global_pool_manifest_path),
                "global_pool_manifest_path": str(self.global_pool_manifest_path),
