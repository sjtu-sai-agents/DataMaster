"""MCTS utility functions for ML-Master

This module contains decay functions for exploration constant adjustment.
Matches the original ML-Master implementation exactly.
"""
...
def piecewise_decay(t, initial_C=1.414, T1=100, T2=200, alpha=0.01, lower_bound=0.7):
    """Piecewise linear decay of exploration constant.

    Args:
        t: Current step
        initial_C: Initial exploration constant
        T1: First transition point
        T2: Second transition point
        alpha: Decay rate
        lower_bound: Minimum value for C
...
def dynamic_piecewise_decay(steps_limit, n_nodes, initial_C, start_time, time_limit, alpha=0.01, lower_bound=0.7, phase_ratios=[0.3, 0.7]):
    """Dynamic segmented decay strategy for exploration constant.

    Adjusts exploration parameter C based on remaining time and the number of nodes already generated.

    Args:
        steps_limit: Maximum number of steps
        n_nodes: Current number of nodes
        initial_C: Initial exploration constant
        start_time: Search start time
        time_limit: Maximum time limit
        alpha: Decay rate
        lower_bound: Minimum value for C
        phase_ratios: Ratios for phase transitions
    ...
    logger.info(f"Dynamic piecewise decay: C={C}, progress={progress:.3f}, phase1_end={phase1_end}, phase2_end={phase2_end}, n_nodes={n_nodes}")
    return C
