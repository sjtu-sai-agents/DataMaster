...
class UCTSearchManager:
    ...
    @staticmethod
    def _extract_submission_score(res: Any) -> float | None:
        if not isinstance(res, dict):
            return None
        for key in ("score", "submission_score", "metric", "value"):
            val = res.get(key)
    ...
    def ingest_result(
        ...
        *,
        ...
        if (
            ...
            node.submission_valid = None
            node.submission_score = None
            node.submission_detail = None
            submission_path = self.submission_dir / f"submission_{node.id}.csv"
            if submission_path.exists():
                ok, res = self.grader(self.exp_id, submission_path)
                node.submission_score = self._extract_submission_score(res)
                if ok:
                    node.submission_valid = res.get("is_valid", True) if isinstance(res, dict) else True
                    if isinstance(res, dict):
                        detail = res.get("result") or res.get("details")
                        if detail:
                            node.submission_detail = str(detail)
                    elif res is not None:
                        node.submission_detail = str(res)
                    if isinstance(res, dict) and not res.get("is_valid", True):
                        node.is_valid = False
                        node.is_buggy = True
                        node.metric = WorstMetricValue()
                        detail = node.submission_detail or "submission 格式非法"
                        logger.info(f"Grader marked node {node.id} as buggy: {detail}")
                        node.analysis = f"{node.analysis or ''}\n[grading] {detail}".strip()
                else:
                    # Grading server connection failed - keep LLM metric unchanged
                    node.submission_detail = str(res)
                    logger.warning(f"Grading server failed for node {node.id}: {res}")
