...
from mlebench.grade import grade_csv as _bench_grade_csv
...
def _create_app(base_dir: Path) -> Flask:
    ...
    def run_validation(submission: Path, competition_id: str) -> dict[str, bool | float | str | None]:
        competition = new_registry.get_competition(competition_id)
        is_valid, message = _bench_validate(submission, competition)
        if not is_valid:
            return {"result": message, "is_valid": False, "score": None}

        report = _bench_grade_csv(submission, competition)
        return {"result": message, "is_valid": True, "score": report.score}
    ...
    def validate():
        ...
        try:
            result = run_validation(submission_path, competition_id)
