...
class WriteDatasetManifestParams(BaseToolParams):
    """Write a red-node dataset manifest and merge it into the global source pool.

    Use this instead of manually writing JSON. The tool validates that `datasets`
    is a non-empty list of objects, normalizes common fields, writes the node
    manifest, and updates the shared global pool manifest visible to later red
    and black nodes.
    """

    name: ClassVar[str] = "write_dataset_manifest"
    manifest_path: str = Field(description="Absolute path to the node dataset_manifest_<node_id>.json")
...
class WriteDatasetManifestTool(BaseTool):
    ...
    def execute(self, session: Any, args_json: str) -> tuple[str, dict[str, Any]]:
        ...
        try:
            from playground.math_posttrain_datatree_v3.core.utils.handoff import (
                load_json_payload,
                merge_global_pool_manifest,
                summarize_global_pool_manifest,
            )
            from playground.math_posttrain_datatree_v3.core.utils.io import write_json
            ...
            manifest_payload = {
                "manifest_id": f"dataset_manifest_{params.node_id}",
                "created_from_node": params.node_id,
                ...
                "source_summary": {
                    ...
                    "agent_summary": params.agent_summary[:500],
                },
            }
...
def _format_manifest_observation(result: dict[str, Any]) -> str:
    ...
    if result.get("manifest_path"):
        lines.append(f"Manifest path: {result.get('manifest_path')}")
