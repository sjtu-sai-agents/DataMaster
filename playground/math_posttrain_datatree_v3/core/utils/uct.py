...
class UCTSearchManager:
    ...
    def pending_execution_nodes(self) -> list[UCTNode]:
        """Return pending execution nodes in current heap order."""
        return [item[2] for item in sorted(self._heap)]

    def pop_execution_node(self, preferred_node_id: str | None = None) -> UCTNode | None:
        if not self._heap:
            return None
        if preferred_node_id:
            for idx, (_, _, node) in enumerate(self._heap):
                if node.id == preferred_node_id:
                    _, _, selected = self._heap.pop(idx)
