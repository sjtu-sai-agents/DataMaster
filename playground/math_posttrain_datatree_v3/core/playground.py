...
from .exp.observer_exp import (
    ObserverExp,
    get_global_advice_path,
    get_observer_history_path,
)
...
@register_playground("math_posttrain_datatree_v3")
class MathPostTrainDataTreeV2Playground(BasePlayground):
    def __init__(self, config_dir: Path | None = None, config_path: Path | None = None):
        if config_path is None and config_dir is None:
            config_dir = Path(__file__).parent.parent.parent.parent / "configs" / "math_posttrain_datatree_v3"
        super().__init__(config_dir=config_dir, config_path=config_path)
        self.logger = logging.getLogger(self.__class__.__name__)
        self.agents.declare("red_agent", "black_agent", "observer_agent")
        self.trajectories: list[dict[str, Any]] = []
        self._observer_selected_next_node_id: str | None = None
    ...
    def _create_worker_agents(self, worker_index: int) -> dict[str, Agent]:
        return {
            ...
            "observer": self.copy_agent(self.agents.observer_agent, new_agent_name=f"math_observer_{worker_index}"),
        }
    ...
    def _serialize_observer_node(self, node: Any) -> dict[str, Any]:
        return {
            "id": node.id,
            "stage": node.stage,
            "parent_id": getattr(getattr(node, "parent", None), "id", None),
            ...
            "search_goal": getattr(node, "search_goal", None),
            ...
            "stdout": _clip_text(getattr(node, "stdout", "") or "", 500),
        }

    def _serialize_observer_tree(self, node: Any) -> dict[str, Any]:
        payload = self._serialize_observer_node(node)
        payload["children"] = [
            self._serialize_observer_tree(child)
    ...
    def _build_observer_context(
        ...
        *,
        ...
        latest_detail = latest_result.get("metric_detail", {}) or {}
        pending_nodes = [
            self._serialize_observer_node(node)
            for node in search_mgr.pending_execution_nodes()
            if getattr(node, "is_buggy", None) is not True
        ]
        latest_handoff_path = (
            latest_detail.get("black_handoff_path")
            or getattr(latest_node, "black_handoff_path", None)
            or getattr(latest_node, "input_black_handoff_path", None)
        )
        latest_inspect_path = latest_detail.get("inspect_report_path") or getattr(latest_node, "inspect_report_path", None)
        global_pool_manifest_path = get_global_pool_manifest_path(task_workspace)
        global_pool_manifest = load_json_payload(global_pool_manifest_path)
        return {
            "current_step": search_mgr.current_step,
            "max_rounds": search_mgr.search_cfg.max_rounds,
            "best_metric": search_mgr.best_metric,
            "best_node_id": search_mgr.best_node.id if search_mgr.best_node else None,
            "latest_completed_node_id": latest_node.id,
            "latest_completed_stage": latest_node.stage,
            "latest_completed_result": {
                "metric": latest_result.get("metric"),
                "exit_code": latest_result.get("exec", {}).get("exit_code"),
                "stdout": _clip_text(latest_result.get("exec", {}).get("stdout", ""), 1200),
                "metric_detail": latest_detail,
            },
            "latest_black_handoff": load_json_payload(latest_handoff_path),
            "latest_inspect_report": read_json(latest_inspect_path, default={}) if latest_inspect_path else {},
            "global_pool_manifest_path": str(global_pool_manifest_path),
            "global_pool_manifest_summary": summarize_global_pool_manifest(global_pool_manifest),
            "pending_nodes": pending_nodes,
            "tree": self._serialize_observer_tree(search_mgr.root),
        }
    ...
    def _run_observer(
        ...
        *,
        ...
        exp = ObserverExp(
            worker_agents["observer"],
            self.session,
            task_workspace,
            task_workspace,
            self.config,
            latest_node,
            get_global_advice_path(task_workspace),
            get_observer_history_path(task_workspace),
        )
        advice = exp.run(task_description, context)
        selected = str(advice.get("selected_next_node_id") or "").strip()
        self._observer_selected_next_node_id = selected or None
        self.logger.info(
            "Observer updated advice path=%s selected_next_node_id=%s reason=%s",
            get_global_advice_path(task_workspace),
            selected or "<uct>",
            _clip_text(str(advice.get("scheduler_reason") or ""), 180),
        )
        return advice
        ...
        *,
        ...
        if stage == "red":
            ...
            exp = RedExp(
                ...
                Path(getattr(node, "global_advice_path", get_global_advice_path(task_workspace))),
            )
    ...
    def _make_child(self, search_mgr: UCTSearchManager, parent: Any, stage: str, task_workspace: Path) -> Any:
        ...
        if stage == "red":
            ...
            child.global_advice_path = str(getattr(parent, "global_advice_path", get_global_advice_path(task_workspace)))
        elif stage == "black":
            if parent.stage == "red":
    ...
    def run(self, task_description: str, output_file: str | None = None) -> dict:
        ...
        while search_mgr.current_step < search_mgr.search_cfg.max_rounds:
            node = search_mgr.pop_execution_node(self._observer_selected_next_node_id)
            self._observer_selected_next_node_id = None
            if node is None:
                break
            try:
            ...
            with state_lock:
                ...
                if node.stage == "seed":
                    ...
                    node.global_advice_path = str(get_global_advice_path(workspace))
                if node.stage == "red":
                    ...
                    node.global_advice_path = getattr(node, "global_advice_path", str(get_global_advice_path(workspace)))
                if node.stage == "black":
                ...
                if node.stage in {"seed", "black"}:
                    observer_advice = self._run_observer(
                        worker_agents=worker_agents,
                        task_workspace=workspace,
                        search_mgr=search_mgr,
                        latest_node=node,
                    ...
                    append_trajectory(
                        self,
                        {
                            "stage": "observer",
                            "node_id": node.id,
                            "selected_next_node_id": observer_advice.get("selected_next_node_id"),
                            "scheduler_reason": observer_advice.get("scheduler_reason"),
                        },
                    )
