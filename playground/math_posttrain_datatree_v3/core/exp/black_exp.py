...
class BlackExp(NodeExp):
    def __init__(
        ...
        global_advice_path: Path | None = None,
        exp_index: int = 0,
    ):
    ...
    def _run_black_data_agent(
        ...
        *,
        ...
        global_advice = load_json_payload(self.global_advice_path)
        ...
        final_turn_prompt = (
            "FINAL TURN. Stop all exploration. Produce final outputs NOW.\n"
            f"Required files:\n"
            f"1. `{self.train_jsonl_path}` (training data in Alpaca JSONL format)\n"
            f"2. `{self.train_config_path}` (training hyperparameters)\n"
            ...
            "Validate with validate_train_data and validate_train_config, then call finish.\n"
        ...
        self.agent._prompt_format_kwargs.update(
            {
                ...
                "global_advice_path": str(self.global_advice_path or ""),
                "global_advice_json": json.dumps(global_advice, ensure_ascii=False, indent=2),
    ...
    def run(self, task_description: str) -> dict:
        ...
        for key, value in training_defaults.items():
            if key != "template" and value not in (None, ""):
                effective_train_config[key] = value
