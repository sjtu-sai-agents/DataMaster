...
# todo 优化：
# 1. ✅ 我希望解析 runs 对应的 trajectory 内容 - 已实现
# 2. ✅ 我希望不同的 node type 使用不同的颜色区分 - 已实现
# 3. ✅ 如果可以，我希望每一个节点都显示验证集的分数和测试集的分数 - 已实现
...
def parse_validation_score(stdout: str) -> Optional[float]:
    """从 stdout 中解析验证集分数"""
    import re
    if not stdout:
        return None
    ...
    # 常见的验证分数模式
    patterns = [
        r"Validation AUC:\s*([\d.]+)",
        r"Validation Score:\s*([\d.]+)",
        r"val_auc\s*[:=]\s*([\d.]+)",
        r"validation.*score[:\s]+([\d.]+)",
        r"val.*score[:\s]+([\d.]+)",
    ]
...
def load_node_tree(run_dir: Path | str) -> Dict[str, Any]:
    """加载 UCT 节点树"""
        ...
        try:
            with open(node_file, 'r', encoding='utf-8') as f:
                ...
                if node and node.get("id"):
                    ...
                    parsed_score = parse_validation_score(stdout)
                    if parsed_score is not None and node.get("metric") is None:
                        node["metric"] = parsed_score
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .score-display { background:linear-gradient(135deg, #1e3a5f 0%, #0d1f33 100%); border:1px solid #2a5a8f; border-radius:8px; padding:12px; margin:10px 0; }
    .score-value { font-size:24px; font-weight:bold; color:#4dabf7; text-align:center; }
    .score-label { font-size:11px; color:#95a0c7; text-align:center; text-transform:uppercase; letter-spacing:1px; margin-top:4px; }
...
<body>
  ...
  <div class=\"layout\">
    <div class=\"left\">
      <svg id=\"svg\"></svg>
      <div class=\"hint\">Wheel: zoom · Drag: pan · Click node: trajectory | Solid circle: has data · Dashed circle: no data</div>
    </div>
  ...
  function nodeColor(node, trajData) {
    ...
    if (node.stage === 'initial') return '#ffd166';  // initial
    const stage = node.stage || '';
    if (stage === 'draft') return '#4dabf7';  // draft
    if (stage === 'debug') return '#ff8787';  // debug
    if (stage === 'improve') return '#51cf66';  // improve
    if (stage === 'data_search') return '#cc5de8';  // data_search
    if (stage === 'black') return '#9775fa';  // black (数据增强)
    if (stage === 'red') return '#ff6b6b';  // red (外部数据)
    return '#8893bd';
  }
    ...
    if (node) {
      ...
      if (node.stage === 'data_search') {
        ...
        if (node.metric !== null && node.metric !== undefined) {
          const metricScore = nodeCard.appendChild(document.createElement('div'));
          metricScore.className = 'score-display';
          metricScore.innerHTML = `
            <div class="score-value">${typeof node.metric === 'number' ? node.metric.toFixed(4) : node.metric}</div>
  ...
  function drawTree(){
    ...
    nodes.append('text')
      ...
      .text(d=>{
        // data_search 阶段显示阶段标识
        if (d.data.stage === 'data_search') {
          return d.data.data_ok ? '✓data' : 'data';
        }
        // 显示验证集分数
        ...
        if (stage === 'root') return 'root';
        if (stage === 'draft') return 'draft';
        if (stage === 'debug') return 'debug';
        if (stage === 'improve') return 'imp';
        return '';
      });
