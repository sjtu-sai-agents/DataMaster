...
def copy_submission(
    ...
    """Copy submission file to a node-specific file if it exists.

    Agent 执行时代码直接将文件保存为 submission_{node_id}.csv，
    所以这里只需要验证文件存在并返回路径。
    """
    import logging
    ...
    # Agent 保存的文件名是 submission_{node_id}.csv
    src = src_dir / f"submission_{node_id}.csv"

    if not src.exists():
        # 如果直接命名的文件不存在，尝试查找 submission.csv（向后兼容）
        fallback_src = src_dir / "submission.csv"
        if fallback_src.exists():
            ...
            try:
                ...
                pass
            logger.info(f"copy_submission: converted legacy format {fallback_src} to {dst}")
            return dst

        logger.warning(f"copy_submission: neither {src} nor submission.csv found in {src_dir}")
        return None

    # 文件已经在正确的位置，直接返回路径
    logger.info(f"copy_submission: found submission file at {src}")
    return src
...
def append_trajectory(playground: Any, record: dict[str, Any], logger: logging.Logger | None = None) -> None:
    """Append a trajectory record to memory (no longer persists to disk)."""
...
def save_node_snapshot(
    ...
    snapshot = {
        ...
        "id": node.id,
        "stage": node.stage,
        "action_type": getattr(node, "action_type", ""),
        ...
        "data_ok": getattr(node, "data_ok", None),
        "data_context": getattr(node, "data_context", None),
...
def _serialize_node(node: Any, search_mgr: UCTSearchManager) -> dict[str, Any]:
    ...
    return {
        ...
        "submission_score": metric_val,  # Keep for backward compatibility
