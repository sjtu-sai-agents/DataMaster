"""Red Experiment Implementation - External Data Search and Integration."""
import logging
...
class RedExp(NodeExp):
    """Red 实验：通过外部搜索接口进行数据搜索、下载新的数据集到本地、引入新的数据源
    ...
    def __init__(self, agent, metric_agent, session, workspace: Path, exp_id: str | None, data_preview: str, node, exp_index: int = 0,
        ...
        super().__init__(agent, metric_agent, session, workspace, exp_id, data_preview, node, exp_index,
                      test_feedback=test_feedback, force_direction=force_direction)
    ...
    def run(
        ...
        logger.info(f"Red node {node_id[:8]}, parent: {parent_id[:8] if parent_id else 'none'}")
        ...
        data_loader_usage_path = Path("playground/data_master/prompts/tools/dataloader_en.md")
        with open(data_loader_usage_path, "r", encoding="utf-8") as file:
            data_loader_usage = file.read()

        # get operation tools readme - red node uses for_datanode tools
        tools_manual_readme_path = Path(
            "playground/data_master/prompts/tools/operate/for_datanode_en.md"
        )
        ...
        if parent_id:
            ...
            if parent_dataloader_path.exists():
                parent_dataloader_code = parent_dataloader_path.read_text(encoding="utf-8")
                
                
        # memory_tree tools readme
        memory_tree_path = (
            "playground/data_master/prompts/tools/memory.md"
        )
        with open(memory_tree_path, "r", encoding="utf-8") as file:
            memory_tree_manual = file.read()
            
        search_tool_path = "playground/data_master/prompts/tools/search.md"
        
        with open(search_tool_path, "r", encoding="utf-8") as file:
            search_tool_manual = file.read()
        search_tool_manual.format(workspace=str(self.workspace))
        ...
        fmt = {
            ...
            "search_tools_manual": search_tool_manual
        }
        
        # TODO Optimize final turn prompt
        ...
        try:
            task = TaskInstance(task_id=f"{node_id}_red", task_type="red", description=task_description, input_data={})
            traj = self.agent.run(task, enable_final_turn=True, enable_final_turn_prompt=final_turn_prompt)
        ...
        if assembled_code:
            ...
            logger.warning("Failed to assemble code, checking for existing code file...")
