...
def _assemble_mode_specific_prompt(
    ...
    if has_code and has_instruction:
        # 模式1: 两者都有
        template_file = Path("playground/ml_master_datatree/prompts/initial/mode_code_and_instruction.md")
        logger.info("Code and Instruction Both!")
    elif has_code:
        # 模式2: 只有代码
        template_file = Path("playground/ml_master_datatree/prompts/initial/mode_code_only.md")
        logger.info("Using initial mode: using initial_code only")
    elif has_instruction:
        # 模式3: 只有指令
        template_file = Path("playground/ml_master_datatree/prompts/initial/mode_instruction_only.md")
        logger.info("Using initial mode: using initial instruction only")
    else:
        # 模式4: 都没有（自由模式）
        template_file = Path("playground/ml_master_datatree/prompts/initial/mode_free.md")
...
class InitialExp(NodeExp):
    ...
    def run(self, task_description: str) -> dict:
        ...
        tools_manual_readme_path = Path(
            "playground/ml_master_datatree/prompts/tools/operate/for_initial_exp_en.md"
        )
        with open(tools_manual_readme_path, "r", encoding="utf-8") as file:
            tools_manual_readme = file.read()

        # memory_tree tools readme
        memory_tree_path = "playground/ml_master_datatree/prompts/tools/memory.md"
