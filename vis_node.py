...
def _build_tree_from_nodes(nodes_by_id: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for nid, raw in nodes_by_id.items():
        ...
        shells[nid] = {
            ...
            "submission_score": _as_float(raw.get("submission_score")),
            "submission_valid": raw.get("submission_valid"),
            "submission_detail": raw.get("submission_detail"),
...
def load_payload(run_dir: Path) -> Dict[str, Any]:
    ...
    for fp in sorted(nodes_dir.glob("*.json")):
        ...
        obj["submission_score"] = _as_float(obj.get("submission_score"))
...
<head>
  ...
  <title>UCT Node Viewer</title>
  <script src=\"https://cdn.jsdelivr.net/npm/d3@7\"></script>
  <style>
    :root { --bg:#0b1020; --panel:#131a2f; --text:#e7ebff; --muted:#95a0c7; --edge:#5f6b98; }
    * { box-sizing:border-box; }
    ...
    .right { width:430px; border-left:1px solid #2a355f; background:rgba(12,18,37,.7); overflow:auto; }
...
<body>
  <div class=\"top\">
    <div class=\"title\">ML-Master UCT Node Viewer</div>
    <div>
  ...
  function updateMeta(){
    metaEl.textContent = `run=${DATA.run_dir} | nodes=${DATA.node_count} | generated_at=${new Date((DATA.generated_at||0)*1000).toLocaleString()}`;
  }
  ...
  function renderDetail(id){
    ...
    const fields = [
      ...
      'is_buggy','is_valid','has_submission',
      'submission_valid','submission_score','submission_detail','submission_file'
    ];
...
def _start_http_server(site_dir: Path, host: str, port: int) -> Dict[str, Any]:
    server_pid_file = site_dir / ".vis_node_server.pid"
    server_log_file = site_dir / "vis_node_server.log"
...
def _start_refresh_daemon(run_dir: Path, site_dir: Path, refresh_seconds: int) -> Dict[str, Any]:
    refresh_pid_file = site_dir / ".vis_node_refresh.pid"
    refresh_log_file = site_dir / "vis_node_refresh.log"
...
def run_refresh_loop(run_dir: Path, site_dir: Path, refresh_seconds: int) -> int:
    ...
    while True:
        try:
            ...
            write_payload(payload, site_dir)
            print(
                f"[vis_node:refresh] updated payload.json at {time.strftime('%Y-%m-%d %H:%M:%S')}",
                flush=True,
            )
        except Exception as exc:
            print(f"[vis_node:refresh] update failed: {exc}", flush=True)
        time.sleep(interval)


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate + auto-deploy ML-Master UCT node visualization")
    p.add_argument("--run-dir", type=str, default=None, help="target run dir (default: latest ml_master_* with logs/uct_nodes)")
    p.add_argument("--site-dir", type=str, default=None, help="output site dir (default: <run_dir>/logs/uct_nodes/vis_node_site)")
    p.add_argument("--host", type=str, default="127.0.0.1", help="http bind host")
    p.add_argument("--port", type=int, default=8787, help="preferred http port")
...
def main() -> int:
    ...
    site_dir = Path(args.site_dir).expanduser().resolve() if args.site_dir else (run_dir / "logs" / "uct_nodes" / "vis_node_site")
    ...
    print(f"[vis_node] run_dir: {run_dir}")
    print(f"[vis_node] html: {index_path}")
    ...
    print(f"[vis_node] deployed: {dep['server']['url']}")
    print(f"[vis_node] server_pid: {dep['server']['pid']}")
    print(f"[vis_node] server_log: {dep['server']['log']}")
    print(f"[vis_node] refresh_pid: {dep['refresh']['pid']}")
    print(f"[vis_node] refresh_log: {dep['refresh']['log']}")
    print(f"[vis_node] refresh_interval: {dep['refresh']['refresh_seconds']}s")
    return 0
