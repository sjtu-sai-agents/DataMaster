...
class GitHubSearchClient:
        ...
        for it in raw.get("items", []):
            ...
            if result_type == "code":
                ...
                normalized["extra"] = {
                    ...
                    "repo": it.get("repository_url", "").replace(
    ...
    # -------------------------
    def search_users(
        self, keyword: str, qualifiers: List[str] = None, per_page: int = 20
    ):
...
def search_users(
    keyword: str, qualifiers: Optional[List[str]] = None, per_page: int = 20
) -> str:
    """
    ...
    try:
        ...
        result = client.search_users(
            keyword=keyword, qualifiers=qualifiers, per_page=per_page
        )
...
def search_pull_requests(
    ...
    try:
        ...
        result = client.search_pr(
            keyword=keyword, repo=repo, user=user, state=state, per_page=per_page
        )
...
def comprehensive_github_search(
    keyword: str,
    search_type: str = "all",
    user: Optional[str] = None,
    language: Optional[str] = None,
    ...
    try:
        ...
        if search_type in ["all", "repos"]:
            repo_result = client.search_repos(
                keyword, user=user, language=language, per_page=per_page
            )
            if repo_result["items"]:
                results.append(f"REPOSITORIES ({repo_result['total']} found):")
                for item in repo_result["items"][:5]:  # Show top 5
                    results.append(
                        f"  - {item['full_name']} - Stars: {item['extra']['stars']} - Language: {item['extra']['language'] or 'Unknown'}"
                    )
                results.append("")

        if search_type in ["all", "code"]:
            code_result = client.search_code(
                keyword, user=user, language=language, per_page=per_page
            )
        ...
        if search_type in ["all", "users"]:
            ...
            if user_result["items"]:
                ...
                for item in user_result["items"][:5]:  # Show top 5
                    results.append(
                        f"  - {item['full_name']} - ID: {item['extra']['id']}"
                    )
                results.append("")
        ...
        if search_type in ["all", "issues"]:
            ...
            if issue_result["items"]:
                ...
                for item in issue_result["items"][:5]:  # Show top 5
                    ...
                    title = item["name"] or "Unknown"
                    results.append(
                        f"  - {repo}#{number}: {title[:50]}{'...' if len(title) > 50 else ''}"
                    )
                results.append("")
        ...
        header = [
            f"Comprehensive GitHub Search Results for '{keyword}':\n",
            "=" * 60,
            "",
        ]
        return "\n".join(header + results)
