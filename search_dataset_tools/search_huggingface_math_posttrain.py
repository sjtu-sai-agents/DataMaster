...
# -*- coding: utf-8 -*-
"""
HuggingFace 数据集搜索 MCP 工具（math_posttrain 隔离版）

只供 math_posttrain_datatree 使用，不影响原始 search_huggingface.py。

这个版本优先修复两类常见不稳定点：
1. 数据集搜索 API (`/api/datasets`) 的 TLS / retry 抖动
2. README 获取时 `hf_hub_download` 触发的 HEAD 请求不稳定
"""
...
mcp = FastMCP("huggingface-search-math-posttrain")

# ============================================================================
# Sandbox 代理 – 优先通过 CPU sandbox 转发请求（可选，通过 HF_SANDBOX_URL 启用）
# ============================================================================
_HF_SANDBOX_URL = os.getenv("HF_SANDBOX_URL", "").rstrip("/")


def _sandbox_call(endpoint: str, payload: dict, timeout: int = 30) -> dict | None:
    """Call sandbox API; return parsed JSON on success, None on failure (triggers fallback)."""
...
def _hf_headers() -> dict[str, str]:
    headers = {
        ...
        "User-Agent": "math-posttrain-hf-mcp/1.0",
    }
...
def _build_session() -> requests.Session:
    ...
    retry = Retry(
        total=int(os.getenv("HF_HTTP_RETRIES", "2")),  # 减少重试次数：4→2
        connect=int(os.getenv("HF_HTTP_RETRIES", "2")),
        read=int(os.getenv("HF_HTTP_RETRIES", "2")),
        backoff_factor=float(os.getenv("HF_HTTP_BACKOFF", "0.5")),  # 减少退避：1.0→0.5
        status_forcelist=(408, 425, 429, 500, 502, 503, 504),
        allowed_methods=frozenset({"GET", "HEAD"}),
        raise_on_status=False,
        respect_retry_after_header=(
            os.getenv("HF_RESPECT_RETRY_AFTER", "0").lower() in {"1", "true", "yes"}
        ),
    )
    # 减小连接池：16→8，在高负载下减少资源竞争
    adapter = HTTPAdapter(
        max_retries=retry,
        pool_connections=8,
        pool_maxsize=8,
        pool_block=False,  # 不阻塞，连接池满时直接创建新连接
    )
    session.mount("http://", adapter)
    session.mount("https://", adapter)
    if os.getenv("HF_DISABLE_ENV_PROXY", "").lower() in {"1", "true", "yes"}:
...
def _request_json(path: str, *, params: dict[str, Any] | None = None, timeout: int = 20) -> Any:
    """发送JSON请求 - 降低默认超时以快速失败"""
    url = f"{_hf_endpoint()}{path}"
    session = _build_session()
    resp = session.get(url, headers=_hf_headers(), params=params or {}, timeout=timeout)
...
def _request_text(url: str, *, timeout: int = 20) -> str:
    """发送文本请求 - 降低默认超时以快速失败"""
    session = _build_session()
    resp = session.get(url, headers=_hf_headers(), timeout=timeout)
    resp.raise_for_status()
    return resp.text
...
def get_hf_client() -> None:
    ...
    if token:
        try:
            ...
            print(f"Warning: HF token login failed: {e}")
    ...
    try:
        ...
        try:
            ...
            return (
                ...
                f"REST API failed with {api_error}; huggingface_hub fallback failed with {hub_error}"
            )
...
def inspect_dataset(
    ...
    try:
        ...
        print(f"Error listing configs for '{dataset_id}': {str(e)}")
        configs = []
...
@mcp.tool()
def get_dataset_configs(dataset_id: str) -> List[str]:
    """
...
def get_dataset_readme(dataset_id: str) -> str:
    ...
    errors: list[str] = []
    try:
        content = _request_text(f"{_hf_endpoint()}/datasets/{dataset_id}/resolve/main/README.md", timeout=25)
        if content.strip():
            return content
    except Exception as e:
        ...
        if isinstance(payload, dict):
            ...
            if isinstance(card_data, dict):
                description = (
                    ...
                    or (card_data.get("dataset_info", {}) or {}).get("description", "")
                )
                if description:
                    return description
    except Exception as e:
...
def get_dataset_sample(
    ...
    try:
        ...
        if split and split in dataset_split_names:
            ...
            print(f"Split: {split} not in {dataset_split_names}")
...
def download_dataset(
    ...
    Args:
        ...
        output_dir (str, optional): Local directory path where dataset files will be saved.
                                   The directory will be created if it doesn't exist.
                                   Default is "./downloaded_datasets".
